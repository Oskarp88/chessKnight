...
const UserProfile = () => {
  ...
  const getUser = async () => {
    try {
      const { data } = await axios.get(`https://chessknigth-22fe0ebf751e.herokuapp.com/api/user/${auth.user._id}`);
      setName(data.name);
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      ...
      const response = await axios.put(`https://chessknigth-22fe0ebf751e.herokuapp.com/api/user/update/${auth.user._id}`, {
         name,
         lastName,
         username,
         country,
